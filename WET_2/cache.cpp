...
/*************************
vector suppurts:
void push_back(const value_type& val): Adds an element to the end of the vector.
void pop_back(): Removes the last element from the vector.
iterator insert(iterator position, const value_type& val): Inserts an element at the specified position.
iterator insert(iterator position, size_type n, const value_type& val): Inserts n copies of val at the specified position.
iterator erase(iterator position): Removes the element at the specified position.
*************************/
cache::cache()
{
    this->Bsize = 0;
...
/*
cache::cache(const cache_t &myCache)
{
    this->Bsize = myCache.Bsize;
    this->Csize = myCache.Csize;
    this->assoc = myCache.assoc;
    this->Ssize = this->Csize - this->BSize - this->assoc;
    this->MemCyc= myCache.MemCyc;

    // init the cache table 
    this->table = new block_t*[int(pow(2,Ssize))];
    for (int i = 0 ; i < pow(2,Ssize) ; i++) {
        this->table[i] = new block_t[int(pow(2,assoc))];
        for (int j = 0; j < pow(2,assoc); i++)
        {
            this->table[i][j] = myCache.table[i][j];
        }
...
void cache::emptySpaceIfNeeded( uint32_t address,
                                cache_t *L1 , 
                                cache_t *L2, 
                                int remove_from) 
{
    if (remove_from == 1)
    {
        ...
        /*set dirty in L2*/
        if (popped_block.valid) { //we popped
            int tag = L2->get_tag(popped_block.address);
            int set = L2->get_set(popped_block.address);
            for (int i = 0 ; i < pow(2,L2->assoc) ; i++)
            {
                if (tag == L2->table[set][i].tag) {
                    (L2->table[set][i]).dirty = true;
                }
            }
        }
    }
...
void cache::dirty(uint32_t address)
{
    int tag = this->get_tag(address);
    int set = this->get_set(address);
    for(int i = 0; i < pow(2,(this->assoc)-1);i++)
    {
        if(this->table[set][i].valid && this->table[set][i].tag == tag)
