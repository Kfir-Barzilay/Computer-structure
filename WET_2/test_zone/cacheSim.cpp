...
int main(int argc, char **argv) {
	...
	while (getline(file, line)) {
		...
		if(DEBUG) {
		if ((operation == 'r') && (!write_allocate)) {cout << "no update is needed" << endl;}
		cout << "------------L1 cache table" << endl;
		cout << "Tag is: " << L1->get_tag(num) << " , Set is: " << L1->get_set(num) << endl;
		cout <<endl;
		L1->printTable();
		cout << "L1 miss: " << missL1 << " , out off: " << missL1+hitL1 << endl << endl;
		
		cout << "------------L2 cache table" << endl;
		cout << "Tag is: " << L2->get_tag(num) << " , Set is: " << L2->get_set(num) << endl;
		cout <<endl;
		L2->printTable();
		cout << "L2 miss: " << missL2 << " , out off: " << missL2+hitL2 << endl << endl;;
...
int read_op(cache_t *L1, cache_t *L2 ,int address)
{
	...
	block_t popped; 
	popped = L1->emptySpaceIfNeeded(address, L1 , L2 , 1);
	if (L2->has(address)) { /*In L2 cache*/
		L1->insert(address);
		L1->update_LRU_counter(address);
		L2->update_LRU_counter(address);
		if (popped.valid && popped.dirty) L2->update_LRU_counter(popped.address);
...
int write_op(cache_t *L1, cache_t *L2 ,int address) 
{
	...
	if(DEBUG){
		cout << "------popped-------   " << std::hex << popped.address << endl;
		if(popped.valid && popped.dirty)	cout << " valid & dirty " << endl;
	} 
	if (popped.valid && popped.dirty) L2->update_LRU_counter(popped.address);
	return SUCCESS;	
}
...
{	
	...
	{
	case 1://hit L1
		if(DEBUG){cout << "HIT L1" <<endl;}
		hitL1 += 1;
		break;
	
	case 2://miss L1, hit L2
		if (DEBUG) cout << "HIT L2" <<endl;
		missL1 += 1;
		hitL2 += 1;
		break;
		
	case 3://miss L1,L2
		if (DEBUG) cout << "NO HIT" <<endl;
		missL1 += 1;
		missL2 += 1;
		break;
